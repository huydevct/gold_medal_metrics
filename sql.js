...
const mostSeasonWins = (season, country) => {
  if(['Summer', 'Winter'].includes(season)){
    return `SELECT year, COUNT(*) AS count FROM GoldMetal WHERE country = '${country}' AND season = '${season}' GROUP BY year ORDER BY COUNT(*) DESC LIMIT 1;`;
  }
  return null;
}
...
const numberGenderMedalLists = (gender, country) => {
...
const numberMenMedalists = country => {
  return numberGenderMedalLists('Men', country);
};
...
const numberWomenMedalists = country => {
  return numberGenderMedalLists('Women', country);
};
...
const orderedMedals = (country, field, isAscending) => {
  ...
  return `SELECT * FROM GoldMetal WHERE country = '${country}' ${orderingString};`;
};
...
const orderedSports = (country, field, isAscending) => {
  ...
  return `SELECT sport, COUNT(sport) AS count, (COUNT(sport)*100/(select COUNT(*) FROM GoldMedal WHERE country = '${country}')) AS percent FROM GoldMetal WHERE country = '${country}' GROUP BY sport ${orderingString};`;
};
